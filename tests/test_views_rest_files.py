...
def test_created_by_population(api_app, db, users):
...
def test_files_get(api_app, db, deposit, files, users):
    """Test rest files get."""
    with api_app.test_request_context():
...
def test_files_get_oauth2(api_app, db, deposit, users, write_token_user_1,
...
def test_files_get_without_files(api_app, db, deposit, users):
...
def test_files_post_oauth2(api_app, db, deposit, files, users,
                           write_token_user_1):
...
def test_files_post(api_app, db, deposit, users):
    """Post a deposit file."""
    real_filename = 'real_test.json'
    content = b'### Testing textfile ###'
    digest = 'md5:{0}'.format(hashlib.md5(content).hexdigest())
...
def test_files_put_oauth2(api_app, db, deposit, files, users,
                          write_token_user_1):
...
def test_files_put(api_app, db, deposit, files, users):
...
def test_file_get(api_app, db, deposit, files, users):
    """Test get file."""
    with api_app.test_request_context():
        # the user is the owner
        with api_app.test_client() as client:
...
def test_file_get_not_found(api_app, db, deposit, users):
    """Test get file."""
    with api_app.test_request_context():
        with api_app.test_client() as client:
            # login
...
def test_file_delete_oauth2(api_app, db, deposit, files, users,
                            write_token_user_1):
...
def test_file_delete(api_app, db, deposit, files, users):
    """Test delete file."""
    with api_app.test_request_context():
        # the user is the owner
        with api_app.test_client() as client:
...
def test_file_put_not_found_bucket_not_exist(api_app, db, deposit, users):
...
def test_file_put_not_found_file_not_exist(api_app, db, deposit, files, users):
...
def test_file_put_oauth2(api_app, db, deposit, files, users,
                         write_token_user_1):
...
def test_file_put(api_app, db, deposit, files, users):
    """PUT a deposit file."""
    with api_app.test_request_context():
        with api_app.test_client() as client:
            old_file_id = files[0].file_id
